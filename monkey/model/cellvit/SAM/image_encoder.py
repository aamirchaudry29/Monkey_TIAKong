    ...
    def __init__(
        ...
        for i in range(depth):
            block = Block(
                ...
                window_size=(
                    window_size if i not in global_attn_indexes else 0
                ),
                input_size=(
                    img_size // patch_size,
                    img_size // patch_size,
...
def get_rel_pos(
    ...
    q_coords = torch.arange(q_size)[:, None] * max(
        k_size / q_size, 1.0
    )
    k_coords = torch.arange(k_size)[None, :] * max(
        q_size / k_size, 1.0
    )
