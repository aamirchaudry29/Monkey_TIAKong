...
class DetectionCellPostProcessor:
    ...
    def post_process_cell_segmentation(
        ...
        for inst_id in inst_id_list:
            ...
            inst_contour = np.squeeze(
                inst_contour[0][0].astype("int32")
            )
        ...
        for inst_id in list(inst_info_dict.keys()):
            ...
            if (
                inst_type == 0
            ):  # ! pick the 2nd most dominant if exist
                if len(type_list) > 1:
                    inst_type = type_list[1][0]
            type_dict = {v[0]: v[1] for v in type_list}
            type_prob = type_dict[inst_type] / (
                np.sum(inst_map_crop) + 1.0e-6
            )
...
def calculate_instances(
    ...
    for i in range(pred_types.shape[0]):
        pred_type = (
            torch.argmax(pred_types, dim=-1)[i].detach().cpu().numpy()
        )
