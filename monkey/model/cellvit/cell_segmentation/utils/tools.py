...
def cropping_center(x, crop_shape, batch=False):
    ...
    if not batch:
        ...
        x = x[
            :, h0 : h0 + crop_shape[0], w0 : w0 + crop_shape[1], ...
        ]
    return x
...
def pair_coordinates(
    ...
    pair_distance = scipy.spatial.distance.cdist(
        setA, setB, metric="euclidean"
    )
    ...
    pairing = np.concatenate(
        [pairedA[:, None], pairedB[:, None]], axis=-1
    )
