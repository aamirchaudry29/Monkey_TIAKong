...
from monkey.model.cellvit.cell_segmentation.utils.post_proc_cellvit import (
    DetectionCellPostProcessor,
)
from monkey.model.cellvit.utils import (
    Conv2DBlock,
    Deconv2DBlock,
    ViTCellViT,
    ViTCellViTDeit,
...
class CellViT(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        assert (
            len(extract_layers) == 4
        ), "Please provide 4 layers for skip connections"
        ...
        self.nuclei_binary_map_decoder = (
            self.create_upsampling_branch(2 + offset_branches)
    ...
    def calculate_instance_map(
        ...
        predictions_["nuclei_type_map"] = predictions_[
            "nuclei_type_map"
        ].permute(0, 2, 3, 1)
        predictions_["nuclei_binary_map"] = predictions_[
            "nuclei_binary_map"
        ].permute(0, 2, 3, 1)
        predictions_["hv_map"] = predictions_["hv_map"].permute(
            0, 2, 3, 1
        )
        ...
        for i in range(predictions_["nuclei_binary_map"].shape[0]):
            ...
            instance_pred = (
                cell_post_processor.post_process_cell_segmentation(
                    pred_map
                )
            )
    ...
    def generate_instance_nuclei_map(
        ...
        for i in range(batch_size):
            instance_type_nuclei_map = torch.zeros(
                (h, w, self.num_nuclei_classes)
            )
            ...
            instance_type_nuclei_maps[i, :, :, :] = (
                instance_type_nuclei_map
            )

        instance_type_nuclei_maps = instance_type_nuclei_maps.permute(
            0, 3, 1, 2
        )
...
class CellViTSAM(CellViT):
    ...
    def __init__(
        ...
        if vit_structure.upper() == "SAM-B":
            ...
            raise NotImplementedError(
                "Unknown ViT-SAM backbone structure"
            )
    ...
    def forward(self, x: torch.Tensor, retrieve_tokens: bool = False):
        ...
        classifier_logits, _, z = self.encoder(x)
        out_dict["tissue_types"] = self.classifier_head(
            classifier_logits
        )
...
class DataclassHVStorage:
    ...
    def get_dict(self) -> dict:
        ...
        if (
            not self.regression_loss
            and "regression_map" in property_dict.keys()
        ):
