    ...
    for i, imgs in enumerate(dataloader):
        ...
        with torch.no_grad():
            for model in models:
                ...
                logits_pred = model(imgs)
                inflamm_logits = logits_pred[:, 0, :, :]
                contour_logits = logits_pred[:, 1, :, :]
                lymph_logits = logits_pred[:, 2, :, :]
                mono_logits = logits_pred[:, 3, :, :]

                _inflamm_prob = activation_dict["head_1"](
                    inflamm_logits
                ).numpy(force=True)
                _contour_prob = activation_dict["head_1"](
                    contour_logits
                ).numpy(force=True)
                _lymph_prob = activation_dict["head_2"](
                    lymph_logits
...
def process_tile_segmentation_masks(
    ...
    lymph_probs_map = np.zeros(shape=(tile_size, tile_size))
    mono_probs_map = np.zeros(shape=(tile_size, tile_size))
    contour_probs_map = np.zeros(shape=(tile_size, tile_size))
    ...
    lymph_probs_map = lymph_probs_map * mask_tile
    mono_probs_map = mono_probs_map * mask_tile
    contour_probs_map = contour_probs_map * mask_tile

    processed_masks = multihead_seg_post_process(
        inflamm_probs_map,
        lymph_probs_map,
        mono_probs_map,
        contour_probs_map,
